...
//		resp.then().log().status();
	...
	public void user_validate_the_id_is_not_null(){
		...
		resp.then().assertThat().body("id", notNullValue()); //This approach of Assert is not used as it is hard to understand. It is just an alternative to below Assert approach and is called MATCHERS approach. However, it is not recommended to use this assert approach.
		
		//check id is greater than zero //Below approach of assert validation is called json path approach.
